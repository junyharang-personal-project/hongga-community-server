...
@RestController @RequestMapping(ServiceURIVersion.NOW_VERSION) public class SystemManualController {
    ...
    @PostMapping("/family/system-manual") public ResponseEntity<DefaultResponse<SystemManualWriteRequestDTO>> writeManual(
            @Valid @RequestBody SystemManualWriteRequestDTO systemManualWriteDTO, @RequestParam ("memberId") Long memberId) {
